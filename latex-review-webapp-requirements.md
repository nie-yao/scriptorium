...
7. 用户逐项执行：
   ...
   - `Reject`：恢复该处原文。
   - 手动编辑：直接修改当前文本。
8. 如果用户手动编辑影响某个修改块，系统将其标记为 `Edited` 或 `Conflict`。
9. 用户完成全部审阅后点击 `Save`。
...
工作区左侧应提供返回项目首页的入口。返回项目首页前，如果当前文件存在未保存内容，应提示用户保存、放弃或取消返回。
...
- 创建项目：输入项目名称和保存位置，创建项目根目录；可选择生成基础 `main.tex` 和 `figures/` 文件夹。
- 打开已有项目：选择已有 LaTeX 项目目录并加入项目列表。
- 切换项目：切换前若当前文件存在未保存内容，应提示保存、放弃或取消切换。
...
- 上传时用户必须选择目标文件夹，或通过拖拽落点确定目标文件夹。
- 若目标路径已有同名文件，应提供 `Replace`、`Keep both`、`Cancel` 三种处理方式；默认不覆盖。
- 上传完成后刷新文件树。
...
- 拖拽过程中应有明确的目标文件夹高亮、不可投放状态和操作失败提示。
...
- hunk 装饰必须跟随 `Working` 文本定位；如果无法可靠定位，应显示 conflict 状态，避免在错误位置标记。
- 当 hunk 被 `accepted` 或 `rejected` 后，编辑器内对应对比装饰应移除或淡化。
- 当 hunk 进入 `edited` 或 `conflict` 状态时，应在编辑器内以不同样式提示该 hunk 已被用户改动或无法定位。
...
review session 创建后，系统应把 hunk 对比渲染到左侧编辑器中。右侧 Review 面板用于集中查看状态和执行操作；左侧编辑器用于在上下文中直接阅读红/绿对比。
...
若用户编辑与某个 pending hunk 重叠，则该 hunk 状态变为 `edited`。

若系统无法可靠定位 hunk，则状态变为 `conflict`，并要求用户手动选择：
...
4. 冲突判断：若匹配失败或多处匹配，则标记为 conflict。
...
系统需要保存应用级项目索引和原有 review session 数据。
...
项目级 review session 数据可继续使用项目目录下的隐藏目录保存：
...
.project-root/
  .latex-review/
    sessions/
      session-id.json
    snapshots/
      file-hash-original.tex
      file-hash-proposed.tex
```

保存内容包括：

- review session 元数据。
- original、proposed、working 快照。
- hunk 状态。
...
Web MVP 后端采用 Node.js + TypeScript 本地服务，负责：
...
POST   /api/projects/:projectId/review/session/:id/hunk/:hunkId/accept
POST   /api/projects/:projectId/review/session/:id/hunk/:hunkId/reject
POST   /api/projects/:projectId/review/session/:id/save

POST   /api/projects/:projectId/compile
GET    /api/projects/:projectId/pdf
```

`/uploads` 应使用 `multipart/form-data`、base64 JSON 或等价的二进制传输方式。每个文件都必须经过路径归一化、扩展名校验、大小限制和冲突处理。
...
Web MVP 阶段若没有原生目录选择器，可先限制创建/打开项目到本地服务启动时授权的 workspace 根目录；桌面端再通过系统目录选择器授权任意项目目录。
...
AI 调用必须通过 `AiSuggestionProvider`。这样 Web MVP 可以先使用本地 HTTP API 或手动导入建议文本，后续桌面端也可以替换为 Tauri/Electron 适配实现。
...
14. Accept、Reject、Keep Edit。
15. 保存最终文件。
16. 在左侧编辑器内显示 hunk 红/绿对比。
17. 通过 `ProjectManagerProvider` 管理项目。
18. 通过 `FileSystemProvider` 读写、上传、移动文件并创建文件夹。
19. 通过 `LatexCompilerProvider` 调用 `latexmk` 编译。
20. 显示编译日志。
21. 通过 `AiSuggestionProvider` 接入 AI 修改建议或手动导入 proposed 文本。
...
12. 用户可以通过拖拽移动项目内文件或文件夹，非法移动会被阻止并提示。
13. 系统可以记录 AI 修改前后的快照。
14. 系统可以把 AI 修改切成多个可审阅 hunk。
15. 用户可以在左侧编辑器中看到每个可定位 hunk 的红/绿对比。
16. 红色旧内容作为编辑器装饰显示，不会被写入 `Working` 文本。
17. 用户可以逐项 Accept 或 Reject。
18. 用户可以在审阅过程中手动修改文本。
19. 手动修改不会破坏未处理 hunk 的基本定位。
20. 无法可靠定位时，系统会显示 conflict，而不是静默误改。
21. 保存后真实文件内容等于用户最终确认的 `Working` 文本。
22. 系统可以通过 `LatexCompilerProvider` 编译 LaTeX 并显示 PDF。
23. UI 层不直接依赖 Node.js 文件系统、Tauri API 或 Electron IPC。
24. 项目管理、文件读写、LaTeX 编译、AI 调用均通过平台接口完成。
25. 所有非 AI 建议生成功能均由确定性代码实现。
